...
const CourseSelection = () => {
  ...
  return (
    <section className="py-12 md:py-24 px-0 md:px-4 w-full max-w-7xl mx-auto overflow-hidden bg-white" id="course-selection">
      <div className="text-center mb-8 md:mb-16 px-4">
        <h2 className="text-3xl md:text-6xl font-black text-[#0f172a] mb-4 tracking-tight">Choose Your Course</h2>
      ...
      <div className="flex justify-center mb-16 px-4">
        <div className="inline-flex flex-wrap items-center bg-[#f8fafc] border border-slate-100 rounded-full py-2 pl-6 pr-2 shadow-sm gap-4 transition-all hover:shadow-md">
          <span className="text-sm md:text-base font-bold text-slate-600 whitespace-nowrap">Select Your Child's Age</span>
      ...
      <div className="max-w-5xl mx-auto px-4 mb-12">
        <div className="flex items-center gap-8 md:gap-14 border-b-2 border-slate-100 overflow-x-auto scrollbar-hide pb-0.5 no-scrollbar relative min-h-[70px]">
          <button 
            onClick={() => setCategoryFilter('All')}
            className={`flex flex-col items-center gap-2 pb-4 text-sm md:text-base font-black transition-all relative whitespace-nowrap px-4 group ${categoryFilter === 'All' ? 'text-primary-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
          ...
          {categories.map((cat) => (
            <button 
              ...
              className={`flex flex-col items-center gap-2 pb-4 text-sm md:text-base font-black transition-all relative whitespace-nowrap px-4 group ${categoryFilter === cat._id ? 'text-primary-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
      ...
      <div className="px-4">
        {filteredCourses.length > 0 ? (
          ...
          <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem] py-24 text-center">
            ...
            <p className="text-slate-400 font-bold max-w-sm mx-auto">Try selecting a different age group or subject category to find the perfect class.</p>
            <Button 
              variant="outline" 
