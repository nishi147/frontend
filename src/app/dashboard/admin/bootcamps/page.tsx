...
export default function AdminBootcamps() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(`Failed to ${editingId ? 'update' : 'create'} bootcamp: ${err.response?.data?.message || err.message}`, "error");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm("Delete Bootcamp?", "Do you really want to remove this bootcamp? All enrollments will be affected.");
  ...
  return (
    <DashboardLayout allowedRoles={['admin']}>
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-800">Bootcamp Management 🎓</h1>
          <p className="text-slate-500 font-bold">Launch long-term intensive learning missions.</p>
        </div>
        <Button onClick={() => {
          setEditingId(null);
          setFormData({ title: '', description: '', date: '', endDate: '', price: 0, venue: '', meetingLink: '', image: '' });
          setIsModalOpen(true);
        }} className="font-black w-full md:w-auto bg-indigo-600 hover:bg-indigo-700">
          <Plus className="mr-2" /> New Bootcamp
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {isLoading ? (
          ...
          bootcamps.map((bc: any) => (
            <Card key={bc._id} className="relative group overflow-visible border-none shadow-sm hover:shadow-xl rounded-[2.5rem] transition-all duration-500 bg-white">
              <div className="h-48 bg-indigo-600 relative overflow-hidden flex items-center justify-center text-7xl rounded-t-[2.5rem]">
                 {bc.image ? (
                   ...
                   <>
                     <div className="text-white opacity-20 font-black uppercase tracking-tighter mix-blend-overlay">BOOTCAMP</div>
              ...
              <CardContent className="p-8 pt-10">
                <div className="flex items-center gap-2 mb-2">
                   ...
                   {bc.instructor && (
                     <span className="text-[10px] font-black text-slate-400">By {bc.instructor.name}</span>
                   )}
                </div>
                ...
                <div className="space-y-4 mb-10 pl-1">
                  <div className="flex items-center gap-4 text-slate-700">
                    ...
                    <div>
                      ...
                      <p className="font-black text-[15px]">{new Date(bc.date).toLocaleDateString()} - {new Date(bc.endDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-slate-700">
                    <div className="w-10 h-10 rounded-full bg-indigo-50 flex items-center justify-center">
            ...
            <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    type="text" required placeholder="e.g. Zoom Live"
                    className="w-full p-4 border-2 rounded-2xl focus:border-indigo-500 outline-none font-bold text-slate-700 bg-slate-50 transition-all"
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Meeting Link (Optional)</label>
                <input 
              ...
              <Button type="submit" className="w-full py-6 font-black text-xl bg-indigo-600 hover:bg-indigo-700 shadow-xl shadow-indigo-100 mt-2 rounded-[1.5rem]">
